...
public class PetitionerlawerParser {
	private static Logger LOGGER = LoggerFactory.getLogger(PetitionerlawerParser.class);
    private final GenericTagger namesPetitionerlawerParser;
    private final GenericTagger namesCitationParser;

    private static final Pattern ET_AL_REGEX_PATTERN = Pattern.compile("et\\.? al\\.?.*$");
	
    public PetitionerlawerParser() {
        namesPetitionerlawerParser = TaggerFactory.getTagger(GrobidModels.NAMES_PETITIONERLAWER);
    ...
    public List<Person> processing(List<LayoutToken> tokens, List<PDFAnnotation> pdfAnnotations, boolean head) {
        ...
        List<Person> fullPetitionerlawers = null;
        try {
            ...
            if (StringUtils.isEmpty(sequence))
                return null;
            GenericTagger tagger = head ? namesPetitionerlawerParser : namesCitationParser;
            String res = tagger.label(sequence);
//System.out.println(res);
            TaggingTokenClusteror clusteror = new TaggingTokenClusteror(head ? GrobidModels.NAMES_PETITIONERLAWER : GrobidModels.NAMES_CITATION, res, tokens);
            ...
            for (TaggingTokenCluster cluster : clusters) {
                ...
                if(pdfAnnotations != null) {
                    for (LayoutToken petitionerlawersToken : cluster.concatTokens()) {
                        for (PDFAnnotation pdfAnnotation : pdfAnnotations) {
                            BoundingBox intersectBox = pdfAnnotation.getIntersectionBox(petitionerlawersToken);
                            if (intersectBox != null) {
                                BoundingBox petitionerlawersBox = BoundingBox.fromLayoutToken(petitionerlawersToken);
                                if (intersectBox.equals(petitionerlawersBox)) {
                                } else {
                                    double pixPerChar = petitionerlawersToken.getWidth() / petitionerlawersToken.getText().length();
                                    ...
                                    if (pdfAnnotation.getDestination() != null && pdfAnnotation.getDestination().length() > 0) {
                                        ...
                                        if (orcidMatcher.find()) {
                                            ...
                                            String newToken = petitionerlawersToken.getText().substring(0, petitionerlawersToken.getText().length() - charsCovered);        
                                            aut.setORCID(orcidMatcher.group(1) + "-"
                                                + orcidMatcher.group(2) + "-" + orcidMatcher.group(3)+ "-" + orcidMatcher.group(4));
                                                petitionerlawersToken.setText(newToken);
                                        }
                                    }
                                }
                            }
                ...
                if (clusterContent.trim().length() == 0)
                    continue;
                if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_MARKER)) {
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_TITLE) || 
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_FORENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_MIDDLENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_SURNAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_PETITIONERLAWER_SUFFIX) || 
            ...
            if (fullPetitionerlawers != null) {
                for(Person author : fullPetitionerlawers) {
