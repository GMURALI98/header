...
public class RespondentlawerParser {
	private static Logger LOGGER = LoggerFactory.getLogger(RespondentlawerParser.class);
    private final GenericTagger namesRespondentlawerParser;
    private final GenericTagger namesCitationParser;

    private static final Pattern ET_AL_REGEX_PATTERN = Pattern.compile("et\\.? al\\.?.*$");
	
    public RespondentlawerParser() {
        namesRespondentlawerParser = TaggerFactory.getTagger(GrobidModels.NAMES_RESPONDENTLAWER);
    ...
    public List<Person> processing(List<LayoutToken> tokens, List<PDFAnnotation> pdfAnnotations, boolean head) {
        ...
        List<Person> fullRespondentlawers = null;
        try {
            ...
            if (StringUtils.isEmpty(sequence))
                return null;
            GenericTagger tagger = head ? namesRespondentlawerParser : namesCitationParser;
            String res = tagger.label(sequence);
//System.out.println(res);
            TaggingTokenClusteror clusteror = new TaggingTokenClusteror(head ? GrobidModels.NAMES_RESPONDENTLAWER : GrobidModels.NAMES_CITATION, res, tokens);
            ...
            for (TaggingTokenCluster cluster : clusters) {
                ...
                if(pdfAnnotations != null) {
                    for (LayoutToken respondentlawersToken : cluster.concatTokens()) {
                        for (PDFAnnotation pdfAnnotation : pdfAnnotations) {
                            BoundingBox intersectBox = pdfAnnotation.getIntersectionBox(respondentlawersToken);
                            if (intersectBox != null) {
                                BoundingBox respondentlawersBox = BoundingBox.fromLayoutToken(respondentlawersToken);
                                if (intersectBox.equals(respondentlawersBox)) {
                                } else {
                                    double pixPerChar = respondentlawersToken.getWidth() / respondentlawersToken.getText().length();
                                    ...
                                    if (pdfAnnotation.getDestination() != null && pdfAnnotation.getDestination().length() > 0) {
                                        ...
                                        if (orcidMatcher.find()) {
                                            ...
                                            String newToken = respondentlawersToken.getText().substring(0, respondentlawersToken.getText().length() - charsCovered);        
                                            aut.setORCID(orcidMatcher.group(1) + "-"
                                                + orcidMatcher.group(2) + "-" + orcidMatcher.group(3)+ "-" + orcidMatcher.group(4));
                                            respondentlawersToken.setText(newToken);
                                        }
                                    }
                                }
                            }
                ...
                if (clusterContent.trim().length() == 0)
                    continue;
                if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENTLAWER_MARKER)) {
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENTLAWER_TITLE) || 
                ...
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENTLAWER_FORENAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENTLAWER_SURNAME) || 
                    ...
                    aut.addLayoutTokens(cluster.concatTokens());
                } else if (clusterLabel.equals(TaggingLabels.NAMES_RESPONDENTLAWER_SUFFIX) || 
            ...
            if (fullRespondentlawers != null) {
                for(Person author : fullRespondentlawers) {
        ...
        try {
            ...
            while (st2.hasMoreTokens()) {
                ...
                if ((line.trim().length() == 0)) {
                    // new author
					if (head)
                    	buffer.append("/t<respondent_lawyers>\n");
					else {
