...
//                List<LayoutToken> tokenizationsHeader = Document.getTokenizationParts(documentHeaderParts, tokenizations);
                ...
                if (resHeader.getExtra() != null) {
                    resHeader.setExtra(resHeader.getExtra());
                }
                ...
                resHeader.setOriginalPetitioners(resHeader.getPetitioners());
                resHeader.setOriginalRespondents(resHeader.getRespondents());
                resHeader.setOriginalPetitionerlawers(resHeader.getPetitionerlawers());
                resHeader.setOriginalRespondentlawers(resHeader.getRespondentlawers());
                ...
                boolean fragmentedPetitioners = false;
                boolean hasMarkerPet = false;
                List<Integer> petitionersBlocks = new ArrayList<>();
                List<List<LayoutToken>> petitionerSegments = new ArrayList<>();
                List<LayoutToken> petitionerLayoutTokens = resHeader.getPetitionersTokensWorkingCopy();
                if (isNotEmpty(petitionerLayoutTokens)) {
                    // split the list of layout tokens when token "\t" is met
                    List<LayoutToken> currentSegment = new ArrayList<>();
                    for(LayoutToken theToken : petitionerLayoutTokens) {
                        if (theToken.getText() != null && theToken.getText().equals("\t")) {
                            if (currentSegment.size() > 0)
                                petitionerSegments.add(currentSegment);
                            currentSegment = new ArrayList<>();
                        } else
                            currentSegment.add(theToken);
                    }
                    ...
                    if (petitionerSegments.size() > 1) {
                        fragmentedPetitioners = true;
                    }
                    for (int k = 0; k < petitionerSegments.size(); k++) {
                        if (petitionerSegments.get(k).size() == 0)
                            continue;
                        List<Person> localPetitioners = parsers.getPetitionerParser()
                            .processingHeaderWithLayoutTokens(petitionerSegments.get(k), doc.getPDFAnnotations());
                        if (localPetitioners != null) {
                            for (Person pers : localPetitioners) {
                                resHeader.addFullPetitioner(pers);
                                if (pers.getMarkers() != null) {
                                    hasMarkerPet = true;
                                }
                                petitionersBlocks.add(k);
                            }
                        }
                    }
                }

                boolean fragmentedRespondents = false;
                boolean hasMarkerResp = false;
                List<Integer> respondentsBlocks = new ArrayList<>();
                List<List<LayoutToken>> respondentSegments = new ArrayList<>();
                List<LayoutToken> respondentLayoutTokens = resHeader.getRespondentsTokensWorkingCopy();
                if (isNotEmpty(respondentLayoutTokens)) {
                    // split the list of layout tokens when token "\t" is met
                    List<LayoutToken> currentSegment = new ArrayList<>();
                    for(LayoutToken theToken : respondentLayoutTokens) {
                        if (theToken.getText() != null && theToken.getText().equals("\t")) {
                            if (currentSegment.size() > 0)
                                respondentSegments.add(currentSegment);
                            currentSegment = new ArrayList<>();
                        } else
                            currentSegment.add(theToken);
                    }
                    ...
                    if (respondentSegments.size() > 1) {
                        fragmentedRespondents = true;
                    }
                    for (int k = 0; k < respondentSegments.size(); k++) {
                        if (respondentSegments.get(k).size() == 0)
                            continue;
                        List<Person> localRespondents = parsers.getRespondentParser()
                            .processingHeaderWithLayoutTokens(respondentSegments.get(k), doc.getPDFAnnotations());
                        if (localRespondents != null) {
                            for (Person pers : localRespondents) {
                                resHeader.addFullRespondent(pers);
                                if (pers.getMarkers() != null) {
                                    hasMarkerResp = true;
                                }
                                respondentsBlocks.add(k);
                            }
                        }
                    }
                }

                boolean fragmentedPetitionerlawers = false;
                boolean hasMarkerPetl = false;
                List<Integer> petitionerlawersBlocks = new ArrayList<>();
                List<List<LayoutToken>> petitionerlawerSegments = new ArrayList<>();
                List<LayoutToken> petitionerlawerlayoutTokens = resHeader.getPetitionerlawersTokensWorkingCopy();
                if (isNotEmpty(petitionerlawerlayoutTokens)) {
                    // split the list of layout tokens when token "\t" is met
                    List<LayoutToken> currentSegment = new ArrayList<>();
                    for(LayoutToken theToken : petitionerlawerlayoutTokens) {
                        if (theToken.getText() != null && theToken.getText().equals("\t")) {
                            if (currentSegment.size() > 0)
                                petitionerlawerSegments.add(currentSegment);
                            currentSegment = new ArrayList<>();
                        } else
                            currentSegment.add(theToken);
                    }
                    ...
                    if (petitionerlawerSegments.size() > 1) {
                        fragmentedPetitionerlawers = true;
                    }
                    for (int k = 0; k < petitionerlawerSegments.size(); k++) {
                        if (petitionerlawerSegments.get(k).size() == 0)
                            continue;
                        List<Person> localPetitionerlawers = parsers.getPetitionerlawerParser()
                            .processingHeaderWithLayoutTokens(petitionerlawerSegments.get(k), doc.getPDFAnnotations());
                        if (localPetitionerlawers != null) {
                            for (Person pers : localPetitionerlawers) {
                                resHeader.addFullPetitionerlawer(pers);
                                if (pers.getMarkers() != null) {
                                    hasMarkerPetl = true;
                                }
                                petitionerlawersBlocks.add(k);
                            }
                        }
                    }
                }

                boolean fragmentedRespondentlawers = false;
                boolean hasMarkerRespl = false;
                List<Integer> respondentlawersBlocks = new ArrayList<>();
                List<List<LayoutToken>> respondentlawerSegments = new ArrayList<>();
                List<LayoutToken> respondentlawerLayoutTokens = resHeader.getRespondentlawersTokensWorkingCopy();
                if (isNotEmpty(respondentlawerLayoutTokens)) {
                    // split the list of layout tokens when token "\t" is met
                    List<LayoutToken> currentSegment = new ArrayList<>();
                    for(LayoutToken theToken : respondentlawerLayoutTokens) {
                        if (theToken.getText() != null && theToken.getText().equals("\t")) {
                            if (currentSegment.size() > 0)
                                respondentlawerSegments.add(currentSegment);
                            currentSegment = new ArrayList<>();
                        } else
                            currentSegment.add(theToken);
                    }
                    ...
                    if (respondentlawerSegments.size() > 1) {
                        fragmentedRespondentlawers = true;
                    }
                    for (int k = 0; k < respondentlawerSegments.size(); k++) {
                        if (respondentlawerSegments.get(k).size() == 0)
                            continue;
                        List<Person> localRespondentlawers = parsers.getRespondentlawerParser()
                            .processingHeaderWithLayoutTokens(respondentlawerSegments.get(k), doc.getPDFAnnotations());
                        if (localRespondentlawers != null) {
                            for (Person pers : localRespondentlawers) {
                                resHeader.addFullRespondentlawer(pers);
                                if (pers.getMarkers() != null) {
                                    hasMarkerRespl = true;
                                }
                                respondentlawersBlocks.add(k);
                            }
                        }
                    }
                }
                ...
                resHeader.setFullPetitioners(resHeader.getFullPetitioners());
                resHeader.setFullRespondents(resHeader.getFullRespondents());
                resHeader.setFullPetitionerlawers(resHeader.getFullPetitionerlawers());
                resHeader.setFullRespondentlawers(resHeader.getFullRespondentlawers());
...
//System.out.println((density-doc.getMinCharacterDensity()) + " " + (doc.getMaxCharacterDensity()-doc.getMinCharacterDensity()) + " " + NBBINS_DENSITY + " " + features.characterDensity);             
    ...
    public BiblioItem resultExtraction(String result, List<LayoutToken> tokenizations, BiblioItem biblio) {
        ...
        for (TaggingTokenCluster cluster : clusters) {
            ...
            if (clusterLabel.equals(TaggingLabels.HEADER_TITLE)) {
            ...
            }else if (clusterLabel.equals(TaggingLabels.HEADER_COURT)) {
                /*if (biblio.getTitle() != null && isDifferentContent(biblio.getTitle(), clusterContent))
                    biblio.setTitle(biblio.getTitle() + clusterContent);
                else*/
                ...
                if (biblio.getCasetype() == null) {
                    biblio.setCasetype(clusterContent);
                }
            } else if (clusterLabel.equals(TaggingLabels.HEADER_EXTRA)) {
                /*if (biblio.getTitle() != null && isDifferentContent(biblio.getTitle(), clusterContent))
                    biblio.setTitle(biblio.getTitle() + clusterContent);
                else*/
                if (biblio.getExtra() == null) {
                    biblio.setExtra(clusterContent);
                }
            ...
            } else if (clusterLabel.equals(TaggingLabels.HEADER_PET)) {
                //if (biblio.getAuthors() != null && isDifferentandNotIncludedContent(biblio.getAuthors(), clusterContent)) {
                if (biblio.getPetitioners() != null) {
                    biblio.setPetitioners(biblio.getPetitioners() + "\t" + clusterNonDehypenizedContent);
                    //biblio.addAuthorsToken(new LayoutToken("\n", TaggingLabels.HEADER_AUTHOR));
                    biblio.collectPetitionersToken(new LayoutToken("\t", TaggingLabels.HEADER_PET));

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectPetitionersTokens(tokens);
                } else {
                    biblio.setPetitioners(clusterNonDehypenizedContent);

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectPetitionersTokens(tokens);
                }
            } else if (clusterLabel.equals(TaggingLabels.HEADER_RESP)) {
                //if (biblio.getAuthors() != null && isDifferentandNotIncludedContent(biblio.getAuthors(), clusterContent)) {
                if (biblio.getRespondents() != null) {
                    biblio.setRespondents(biblio.getRespondents() + "\t" + clusterNonDehypenizedContent);
                    //biblio.addAuthorsToken(new LayoutToken("\n", TaggingLabels.HEADER_AUTHOR));
                    biblio.collectRespondentsToken(new LayoutToken("\t", TaggingLabels.HEADER_RESP));

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectRespondentsTokens(tokens);
                } else {
                    biblio.setRespondents(clusterNonDehypenizedContent);

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectRespondentsTokens(tokens);
                }
            } else if (clusterLabel.equals(TaggingLabels.HEADER_PETLAWERS)) {
                //if (biblio.getAuthors() != null && isDifferentandNotIncludedContent(biblio.getAuthors(), clusterContent)) {
                if (biblio.getPetitionerlawers() != null) {
                    biblio.setPetitionerlawers(biblio.getPetitionerlawers() + "\t" + clusterNonDehypenizedContent);
                    //biblio.addAuthorsToken(new LayoutToken("\n", TaggingLabels.HEADER_AUTHOR));
                    biblio.collectPetitionerlawersToken(new LayoutToken("\t", TaggingLabels.HEADER_PETLAWERS));

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectPetitionerlawersTokens(tokens);
                } else {
                    biblio.setPetitionerlawers(clusterNonDehypenizedContent);

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectPetitionerlawersTokens(tokens);
                }
            } else if (clusterLabel.equals(TaggingLabels.HEADER_RESPLAWERS)) {
                //if (biblio.getAuthors() != null && isDifferentandNotIncludedContent(biblio.getAuthors(), clusterContent)) {
                if (biblio.getRespondentlawers() != null) {
                    biblio.setRespondentlawers(biblio.getRespondentlawers() + "\t" + clusterNonDehypenizedContent);
                    //biblio.addAuthorsToken(new LayoutToken("\n", TaggingLabels.HEADER_AUTHOR));
                    biblio.collectRespondentlawersToken(new LayoutToken("\t", TaggingLabels.HEADER_RESPLAWERS));

                    List<LayoutToken> tokens = cluster.concatTokens();
                    biblio.collectRespondentlawersTokens(tokens);
                } else {
                    biblio.setRespondentlawers(clusterNonDehypenizedContent);
...
//                    } else
    ...
    public StringBuilder trainingExtraction(String result, List<LayoutToken> tokenizations) {
        ...
        while (st.hasMoreTokens()) {
            ...
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<case_number>", "<case_number>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<judgement_date>", "<judgement_date>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<judge>", "<judge>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<document_type>", "<document_type>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<extra>", "<extra>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<author>", "<byline>\n\t<docAuthor>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<petitioner>", "<petitioner>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<respondent>", "<respondent>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<petitioner_lawer>", "<petitioner_lawer>", addSpace);
            }
            if (!output) {
                output = writeField(buffer, s1, lastTag0, s2, "<respondent_lawer>", "<respondent_lawer>", addSpace);
            }
            if (!output) {
    ...
    private void testClosingTag(StringBuilder buffer, String currentTag0, String lastTag0) {
        if (!currentTag0.equals(lastTag0)) {
            ...
            if (lastTag0.equals("<title>")) {
                ...
                buffer.append("</court>\n"); 
            } else if (lastTag0.equals("<case_number>")) {
                buffer.append("</case_number>\n"); 
            } else if (lastTag0.equals("<judgement_date>")) {
                buffer.append("</judgement_date>\n"); 
            } else if (lastTag0.equals("<judge>")) {
                buffer.append("</judge>\n"); 
            } else if (lastTag0.equals("<document_type>")) {
                buffer.append("</document_type>\n"); 
            } else if (lastTag0.equals("<extra>")) {
                buffer.append("</extra>\n"); 
            } else if (lastTag0.equals("<author>")) {
                buffer.append("</docAuthor>\n\t</byline>\n");
            } else if (lastTag0.equals("<petitioner>")) {
                buffer.append("</petitioner>");
            } else if (lastTag0.equals("<respondent>")) {
                buffer.append("</respondent>");
            } else if (lastTag0.equals("<petitioner_lawer>")) {
